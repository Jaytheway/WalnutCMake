...
namespace Walnut {
	...
	void Log::Init()
	{
		...
		std::vector<spdlog::sink_ptr> walnutSinks =
		{
		...
		walnutSinks[0]->set_pattern("[%T] [%l] %n: %v");
		appSinks[0]->set_pattern("[%T] [%l] %n: %v");

#if WL_HAS_CONSOLE
		walnutSinks[1]->set_pattern("%^[%T] %n: %v%$");
		appSinks[1]->set_pattern("%^[%T] %n: %v%$");
#endif

		s_CoreLogger = std::make_shared<spdlog::logger>("WALNUT", walnutSinks.begin(), walnutSinks.end());
